...
describe("the lesson, followed", () => {
  for (const lang of LANGS) {
    it(`gets a player who does what the window says out through the airlock, in ${lang}`, () => {
      setLang(lang);
      try {
        let turns = 0;
        for (const seed of LESSON_SEEDS) {
          const run = followLesson(seed);
          expect(run.stuck, `${lang}, seed ${seed}`).toBeUndefined();
          expect(lessonStatus(run.game)?.over, `${lang}, seed ${seed}: the lesson is not over`).toBe(true);
          expect(voyageOf(run.game).state[0]!.sold, `${lang}, seed ${seed}: the hull was not taken`).toBe(true);
        ...
        console.log(`${lang}: ${LESSON_SEEDS.length}/${LESSON_SEEDS.length} lessons over, ${(turns / LESSON_SEEDS.length).toFixed(1)} turns each`);
      } finally {
