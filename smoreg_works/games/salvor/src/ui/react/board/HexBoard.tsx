...
const PROP: Record<string, { c: string; dash: [number, number]; fill?: string }> = {
  /* A compartment the ship blew open. Its own rhythm rather than the hazard's,
     because it is not one thing that happened to the room — it is the room:
     what was in it is gone and every route the game plans goes round it. */
  blown: { c: "var(--sv-burned)", dash: [2, 5], fill: "var(--sv-red-wash)" },
...
/**
 * What ink a thing is drawn in.
 *
 * Three colours and no fourth. Red is the machines' and has been since the
 * board was drawn; green is the job's — one of the three systems that end the
 * hull; amber is everything else. A system already up keeps the colour and
 * loses the weight, so the three read as one job with some of it done rather
 * than as two kinds of thing. The same rule the other two drawings keep
 * (`ui/web/tiles.ts`, `toneOf`), which is why they cannot disagree about what
 * green means.
 */
export function inkOf(thing: BoardThing): string {
  if (thing.hostile === true) return "var(--sv-bad)";
  if (thing.goal === undefined) return "var(--sv-amber)";
  return thing.goal === "up" ? "color-mix(in oklab, var(--sv-good) 62%, transparent)" : "var(--sv-good)";
}
...
export interface BoardThing {
  ...
  hostile?: true;
  /**
   * One of the three systems the hull is neutralised by, and whether it is up:
   * `"down"` while there is work to do in there, `"up"` once it is raised.
   *
   * The same flag the other two drawings carry (`ui/schematic.ts`,
   * `SchematicThing`), for the same reason: the hull's whole job should be
   * findable at a glance rather than three marks in the ink the crates and the
   * bodies are already in — «выделяй значки с целями отдельным цветом,
   * например зелёным» (the owner). Absent on everything else.
   */
  goal?: "up" | "down";
...
export interface BoardRoom {
  ...
  /**
   * What fills the compartment, once the drone knows of it, and the word the
   * codex calls it by — the same pair the SVG hull tints its floor with
   * (`ui/web/hex-svg.ts`). A compartment a charge has already blown carries it
   * too: the blast writes over whatever hazard was there.
   */
  hazard?: { id: string; word: string };
  /**
   * Turns left on the charge the ship set in here (`systems/alert.ts`,
   * `fuseIn`). Drawn whatever the drone knows of the compartment, because the
   * panel names it either way: a charge is the ship talking, not a discovery.
   */
  charge?: number;
}
...
export interface BoardDoor {
  ...
  a: number;
  b: number;
  /** What the engine calls it — the key the ink is chosen by, never printed. */
  state: string;
  /** The same state as a word, in the language that is on (`model.ts`). */
  word: string;
  /**
 ...
 *
 * The chip keeps the one name the bodies share, unpluralised. It used to grow
 * an `s` — "scouts ×3" — which is an English plural glued onto a name that
 * comes out of a table in three languages, and neither Spanish nor Russian
 * builds one that way. The count is printed beside the name as `×3` anyway, so
 * the plural said twice what the number says once, and the number says it in
 * every language.
 */
interface Chip {
  kind: string;
  ...
  return (
    <Popover
      ...
      style={{
        ...
        gap: 10,
        padding: "6px 10px",
        /* Wide enough for a Russian name and its verb, and no wider: it was
           `nowrap`, which in English merely made a long popover and in Russian
           ran `ремонтный бот` and `разобрать` off the edge of the board. */
        maxWidth: 260,
      }}
    >
      <span
      ...
      >
        {chip.n > 1 ? `${it.name} ×${chip.n}` : it.name}
      </span>
      {it.hostile === true ? (
          ...
          {[0, 1, 2].map((i) => (
            <span
              ...
              style={{
                width: 7,
                height: 12,
                background: i < (it.threat ?? 1) ? "var(--sv-bad)" : "var(--sv-plate-lit)",
              }}
            />
          ))}
        </span>
      ) : it.note === undefined ? null : (
        <span data-sc style={{ font: "var(--sv-body)", color: "var(--sv-soft)" }}>
          {it.note}
        </span>
      )}
      {it.work === undefined ? null : (
        ...
        <span style={{ display: "flex", gap: 2 }}>
          {Array.from({ length: it.work.of }, (_, i) => (
            <span
              key={i}
              style={{
                width: 6,
                height: 12,
                background: i < it.work!.done ? "var(--sv-good)" : "var(--sv-plate-lit)",
              }}
            />
          ))}
        </span>
...
function HexTile({
  ...
  return (
    ...
    >
      ...
      {/* What was done to the compartment rather than left in it: the word for
          whatever fills it, over the top of the cell, and the charge burning in
          it counting down at the foot. Both stand clear of the three slots so
          nothing they say moves the contents about, and the count is drawn on a
          compartment the drone has never looked into — the ship announces its
          own charges (`systems/alert.ts`, `panelLines`). */}
      {room.hazard === undefined || !s.shows ? null : (
        <div
          style={{
            position: "absolute",
            left: 0,
            ...
            letterSpacing: ".1em",
            textTransform: "uppercase",
            color: ring?.c ?? "var(--sv-bad)",
          }}
        >
          {room.hazard.word}
        </div>
      )}
      {room.charge === undefined ? null : (
        <div
          style={{
            position: "absolute",
            left: "50%",
    ...
    >
      ...
      <div
        style={{
          ...
          color: "var(--sv-soft)",
          pointerEvents: "none",
          /* Half the play area at the width the owner plays in (1366), so the
             Russian line wraps to two rather than running under the readout. */
