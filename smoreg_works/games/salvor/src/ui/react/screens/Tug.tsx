...
export function TugOrders({
  ...
  return (
    ...
    >
        <Panel title={tug.callsign} stencil={t("word.tug")}>
          <div style={{ display: "flex", gap: 18, alignItems: "stretch" }}>
            ...
            <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 3 }}>
              <Figure label={t("react.stat.banked")} value={tug.account.credits} big />
              <Figure label={t("react.stat.held")} value={tug.account.loot} />
              <Figure label={t("react.stat.keys")} value={tug.account.keys} />
              <Figure label={t("react.stat.hold")} value={tug.account.hold} />
              <Figure label={t("react.stat.sortie")} value={tug.account.sortie} />
              <Figure label={t("react.stat.lost")} value={tug.lost} />
            </div>
                ...
                {tug.derelict.sold ? (
                  ...
                  <>
                    <Tag tone={tug.derelict.online === tug.derelict.of ? "good" : "warn"}>
                      {t("react.tug.systems", { up: tug.derelict.online, of: tug.derelict.of })}
                    </Tag>
                    <Tag tone="amber">{t("word.cr", { n: tug.derelict.price })}</Tag>
                  </>
                )}
              </div>
            </div>
        ...
        {tug.next === undefined ? null : <NextStop next={tug.next} />}

        <Panel title={level === null ? t("panel.actions") : level} stencil={t("room.dock")} fill>
...
function OfferList({
  ...
  return (
    <div ref={ref} style={{ display: "flex", flexDirection: "column", gap: 1 }}>
      {offers.length === 0 ? (
        <div style={{ font: "var(--sv-body)", color: "var(--sv-soft)" }}>{t("why.stock.none")}</div>
      ) : (
        offers.map((offer) => (
          <div key={offer.index}>
        ...
        return (
          ...
          >
            <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
              ...
              >
                {hull.on ? t("react.dock.rails") : t("word.cr", { n: hull.price })}
              </span>
            </div>
            ...
            {open ? (
              <div style={{ marginTop: 5, display: "flex", gap: 6, flexWrap: "wrap" }}>
                <Tag tone="amber">{t("react.hull.core", { n: hull.core })}</Tag>
                <Tag tone="neutral">{t("react.hull.slots", { n: hull.slots })}</Tag>
                {hull.speed === undefined ? null : (
                  <Tag tone="neutral">{t("react.hull.speed", { n: hull.speed })}</Tag>
                )}
              </div>
            ) : null}
          </div>
...
export function DockPreview({
  ...
  return (
    <Panel title={t("room.dock")} stencil={t("react.rack.drones")}>
...
/**
 * The hulls the tug can fly to, and what each one is.
 *
 * There is no contracts menu on this screen and there is not meant to be: since
 * G90 F a contract is signed with the hull it is signed for, on the single row
 * of the list that does both. That row can print a name and a price and no
 * more, so the reading the choice actually needs — how big the hull is, what it
 * sells for, how many machines are in it, what it carries — stands here, beside
 * the list rather than behind a second one. The same four facts the terminal's
 * own board prints under its jump row (`ui/tugboard.ts`, `candidateLine`).
 *
 * Nothing here is pressable. Choosing is done on the list, where every other
 * price on this screen is spent.
 */
function NextStop({ next }: { next: NonNullable<TugModel["next"]> }): ReactElement {
  return (
    <Panel title={t(next.here ? "react.next.here" : "react.next.next")} stencil={`${next.stop}/${next.of}`}>
      <div ref={useLines([next.stop, next.hulls.length])} style={{ display: "flex", flexDirection: "column", gap: 7 }}>
        {next.hulls.map((hull) => (
          <div key={hull.name} style={{ borderLeft: "2px solid var(--sv-line)", paddingLeft: 9 }}>
            <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
              <span
                data-sc
                style={{
              ...
              >
                {t("word.cr", { n: hull.price })}
              </span>
            </div>
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 4 }}>
              <Tag tone="neutral">{t("react.next.rooms", { n: hull.rooms })}</Tag>
              <Tag tone="warn">{t("react.next.machines", { n: hull.machines })}</Tag>
              {hull.dangers.map((danger) => (
                <Tag key={danger} tone="bad">
                  {danger}
                </Tag>
              ))}
            </div>
