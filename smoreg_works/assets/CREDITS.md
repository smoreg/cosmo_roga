...
| `games/salvor/public/audio/sfx/*.ogg` — семь эффектов | предоставлены владельцем проекта (исходные WAV вне репозитория) | пережимаются `node tools/sfx/bake.mjs` в games/salvor; сам каталог в `.gitignore`, как музыка и палуба | как у репозитория |
| `games/salvor/assets/icon-256.png` — иконка 256×256 | этот проект | растр того же `public/favicon.svg` (`magick -background none ... -resize 256x256`); сам знак — `metal-scales` с game-icons.net | **CC BY 3.0** — как у остальных иконок |
| `games/salvor/assets/cover.png` — обложка для itch | этот проект | рендер настоящего борта из игры: `games/salvor/cover.html` + `tools/cover/shoot.mjs`, сид в коде. Плитка внутри — та же CC BY-NC, что и палуба | как у репозитория (плитка — CC BY-NC) |
