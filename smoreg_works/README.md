...
- **`games/salvor`** — то, что сдаётся. Дрон-мусорщик на графе отсеков: корабль-дереликт
  генерируется отсеками и дверями, урон приходит в модули рига, а не в игрока, обезвредить
  корабль — поднять три системы и уйти через шлюз. Рейс — три корпуса, последний буксир отца.
