...
- `hexmap.html` — карта абордажа: план растеризуется, заливкой снаружи
  отделяется вакуум, замкнутые куски становятся зонами свободного движения,
  сверху ложится сетка сот (размер соты — ползунок), соты — узлы с содержимым,
  двери ищутся по тонким стенам между зонами. Решётка та же, что в
  `smoreg_works/experiments/hullforms/hexgrid.mjs`;
- `geomorph-core.js` — общая модель для обеих страниц, без DOM.
