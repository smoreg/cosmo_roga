...
export function frames(
  steps,
  { frame = FRAME, delay = 0, onDone, skip = prefersReducedMotion() } = {},
) {
  if (skip) {
    steps.forEach((s) => s());
    if (onDone) onDone();
...
export function scrambleReveal(
  ...
  if (skip) {
    list.forEach((el) => {
      el.textContent = el.dataset.text != null ? el.dataset.text : el.textContent;
    });
    if (onDone) onDone();
    return handle([]);
  }
  ...
  list.forEach((el, i) => {
    ...
    const noise = (ch) =>
      chars ? chars[(Math.random() * chars.length) | 0] : noiseChar(ch, mono, blockChance);
...
export function teleport(
  ...
  const scr = () => {
    el.textContent = randomScramble(n, chars);
  };
  return frames(
    [
      scr,
