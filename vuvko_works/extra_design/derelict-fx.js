  ...
  steps.forEach((pos, i) => {
    timers.push(setTimeout(() => {
      ...
      const giv = setInterval(() => {
        ghost.textContent = randomScramble(faceLen, chars);
      ...
      place(el, pos);
      if (scrambleFace) el.textContent = randomScramble(faceLen, chars);
    }, i * frame));
  });

  const tail = steps.length * frame;
  timers.push(setTimeout(() => { el.textContent = label; if (onDone) onDone(); }, tail + frame));
  return handle(timers, intervals);
}
...
/**
 * Teleport — dissolve into noise here, reassemble there. Longer and more
 * expensive than blink; use it for a jump between levels or ships rather than
 * for a step inside one.
 */
export function teleport(el, to, {
  place = defaultPlace, frame = FRAME, label = el && el.textContent,
  chars = TOKEN_CHARS, onDone, skip = prefersReducedMotion()
} = {}) {
  if (!el) { if (onDone) onDone(); return handle([]); }
  if (skip) { place(el, to); el.textContent = label; if (onDone) onDone(); return handle([]); }

  const n = (label || '··').length;
  const scr = () => { el.textContent = randomScramble(n, chars); };
  return frames([
    scr, scr, scr,
    () => { el.style.opacity = '0'; },
